...
| [**Synchronization**](docs/sync.md) | Distributed `Lock` and `TaskLock`. Redis, PostgreSQL, SQLite, Kubernetes, in-memory. |
| [**Coordination**](docs/coordination.md) | `LeaderElection` with a record-carrying lease (holder, times, transitions, metadata). Redis, PostgreSQL, Kubernetes Lease, in-memory. |

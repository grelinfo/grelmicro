...
def test_programmatic_path_uses_kwargs() -> None:
    ...
    health = HealthChecks(timeout=TIMEOUT_KWARG, cache_ttl=CACHE_TTL_KWARG)
    assert health._config.timeout == TIMEOUT_KWARG
    assert health._config.cache_ttl == CACHE_TTL_KWARG
...
def test_declarative_path_uses_from_config() -> None:
    ...
    health = HealthChecks.from_config(cfg)
    assert health._config is cfg
...
def test_kwargs_override_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("GREL_HEALTH_TIMEOUT", str(TIMEOUT_ENV))
    health = HealthChecks(timeout=TIMEOUT_KWARG)
    assert health._config.timeout == TIMEOUT_KWARG
...
def test_env_prefix_override(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    health = HealthChecks(env_prefix="MYAPP_HEALTH_")
    assert health._config.timeout == TIMEOUT_ENV
...
def test_env_load_false_ignores_env(
    ...
    assert health._config.timeout == DEFAULT_TIMEOUT
...
def test_zero_config_uses_health_defaults(
    ...
    health = HealthChecks()
    assert health._config.timeout == DEFAULT_TIMEOUT
    assert health._config.cache_ttl == DEFAULT_CACHE_TTL

...
The bulkhead opens its `uses=` on first entry and closes them when the app shuts down, so an active `Grelmicro` app is required. The scope belongs to that app, which holds the record of it, so a later app in the same process opens the scope again from the start. Entering the bulkhead once its app has shut down raises rather than running against closed providers. List the Provider before the Component that borrows it, exactly as in `Grelmicro(uses=[...])`.

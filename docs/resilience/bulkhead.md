...
The bulkhead opens its `uses=` on first entry and closes them when the app shuts down, so an active `Grelmicro` app is required. The scope belongs to that app, and the app holds the record of it. A later app in the same process opens the scope again from the start, and two apps running at once each get their own. List the Provider before the Component that borrows it, exactly as in `Grelmicro(uses=[...])`.

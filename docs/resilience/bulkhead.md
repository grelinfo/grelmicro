...
The bulkhead opens its `uses=` on first entry and closes them when the app shuts down, so an active `Grelmicro` app is required. Something has to open the Provider a Component borrows: list it here, list it on the app, or open it yourself. A Component that opens ahead of the Provider it borrows says which one to move. A scope that opens during startup sits below whatever the app enters after it. So a Provider the app opens later closes first, and a Component in the scope that borrows it closes against a dead client. List that Provider ahead of whatever enters the bulkhead. Listing it in both places is fine: whichever opens it first owns it. A Component in the list claims its whole kind, so a bare Provider beside it fills only the other kinds it serves, exactly as on the app. The bulkhead scope runs on the app's exit stack, so it never adopts a Provider you left out, where `Grelmicro(uses=[...])` would. Order inside `uses=` does not matter: a Provider listed after the Component that borrows it is moved ahead of it.

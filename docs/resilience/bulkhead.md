...
The scope belongs to one app run: whichever enters the bulkhead first. A later run in the same process opens every item again from the start. A run that overlaps the owner borrows the open items instead of opening a second set, and gives them up when the owner closes them, so give overlapping apps their own `Bulkhead` when their lifetimes differ. Entering when no run has it open, because it is still opening, closing, or already gone, raises `OutOfContextError`, or `NoActiveAppError` when no app is active at all, and says what to wait for.

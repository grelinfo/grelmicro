...
The bulkhead opens its `uses=` on first entry and closes them when the app shuts down, so an active `Grelmicro` app is required. The scope belongs to that app, which holds the record of it, so a later app in the same process enters every item again from the start. One bulkhead serves one app at a time: entering it once its app has shut down raises, and so does sharing it with a second app that runs at the same time. Re-entering an item does not always hand it back ready: `PostgresProvider` and `SqliteProvider` rebuild their pool, while `RedisProvider` and `ValkeyProvider` keep the client they built at construction, exactly as in `Grelmicro(uses=[...])`. List the Provider before the Component that borrows it, exactly as in `Grelmicro(uses=[...])`.

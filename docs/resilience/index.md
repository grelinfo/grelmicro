...
- [**Circuit CircuitBreakers**](circuit-breaker.md): detect repeated failures in a downstream call and stop sending requests long enough for it to recover. This prevents cascading failures.
...
| Pattern | Use when |
|---|---|
| **Circuit CircuitBreakers** | The caller is hitting an external dependency (DB, API, third-party) that can fail or stall. |

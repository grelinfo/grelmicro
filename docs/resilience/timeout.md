...
The error carries the deadline it enforced, so a caller reading `exc.timeout` knows the wall it hit. Over HTTP it becomes a `504` [problem detail](../http/errors.md) carrying the same number.

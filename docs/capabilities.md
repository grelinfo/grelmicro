...
- **Redis** when you already run Redis and want the lowest-latency distributed option. `RedisProvider` switches to Sentinel or Cluster from the URL scheme alone (see [Providers](providers/index.md)). On Cluster the cache and lock prefixes need a hash tag.

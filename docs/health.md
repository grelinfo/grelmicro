...
Pass `include_in_schema=True` to publish them, with the `/healthz` response model and its `503` response:

```python
--8<-- "health/openapi.py"
```

This decides what the schema publishes, not what is reachable. To keep an endpoint from answering at all, gate it with `healthz_dependencies` or leave it off your ingress.

...
* 🐛 `CircuitBreaker.from_thread` refuses an entry made from a thread that runs an event loop instead of deadlocking on it. It hands the admission to the loop and blocks on the result, which never arrives when the caller is the loop itself. Reach a breaker from sync code through `asyncio.to_thread(...)`, or use `async with breaker:` from async code. ([#689](https://github.com/grelinfo/grelmicro/issues/689))

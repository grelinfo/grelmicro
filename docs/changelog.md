...
### Fixed
* 🐛 A query parameter that addresses a row rather than holding a secret keeps its value in a log. `sort_key`, `partition_key`, `routing_key`, `cache_key`, `shard_key` and their camel-case spellings read as credentials to the `*_key` rule and never carry one. Any other name ending in `key` is still masked. ([#833](https://github.com/grelinfo/grelmicro/issues/833))
* 🐛 A colon inside a path is no longer read as a credential. `https://h//path:x@y` carries no userinfo and is left alone, while a protocol-relative `//user:pw@host` is still masked. ([#833](https://github.com/grelinfo/grelmicro/issues/833))

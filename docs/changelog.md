...
### Security
* 🔒 An idempotency key can no longer replay a private response across callers when no identity-aware `key_maker` is configured. Requests carrying `Authorization` or `Cookie`, authenticated Starlette scopes, and FastAPI routes with dependencies, including ordinary custom-header authentication, bypass the unscoped default and always run the application, so route-level authentication cannot be skipped by a replay. Discovery covers FastAPI wrapped directly, behind ASGI middleware, or mounted under another ASGI application while keeping parent dependencies outside a mounted public app. Starlette authentication configured lazily on an app or inside a mount is detected path by path, so a protected mount bypasses the default without disabling public siblings. A response-cache declaration behind middleware wrapped around or configured inside a mounted application stays inside that boundary so a parent hit cannot skip it. A required idempotency key is still enforced before that bypass. The built-in key format is versioned so an upgraded or rolling-deployment process cannot read unscoped entries written by the vulnerable format.
* 🔒 Access logs retain query parameter names but mask every value. OAuth/OIDC tokens, authorization codes, presigned URL signatures, cloud credentials, and application-specific bearer capabilities no longer depend on an incomplete credential-name denylist to stay out of logs. URL and configuration redaction also recognizes qualified and camel-case password, key, token, credential, secret, and signature names, masks credential parameters in fragments, and fully masks ambiguous scheme-less or malformed-scheme passwords containing colons, commas, or `@`.

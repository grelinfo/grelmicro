...
* ✨ `RateLimitedRequests()` documents the `429`, and the `RateLimit`, `RateLimit-Policy` and `Retry-After` fields it carries, on every operation in the OpenAPI schema, so a generated client has a branch for it. Every operation rather than the metered ones, because what is metered is tuned live and the schema is not. Pass `openapi=False` to leave the schema alone. ([#813](https://github.com/grelinfo/grelmicro/issues/813))

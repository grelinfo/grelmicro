...
### Breaking
* 💥 `TaskRouter(tasks=[...])` and `Tasks(tasks=[...])` add each task through `add_task` instead of taking the list as it is, so the guard covers a task built by hand as well as one added by a decorator. A subclass that overrides `add_task` now sees those tasks during construction, and has to set its own state before it calls `super().__init__(...)`. The router itself is whole before the first task is added. The list is copied rather than kept, so changing it afterwards no longer changes the router. ([#788](https://github.com/grelinfo/grelmicro/issues/788))
* 💥 Every grelmicro decorator refuses a function a registrar already holds. `@tasks.every`, `@tasks.cron`, `tasks.add_task`, `@health.check`, and `@outbox.handler` record the function they are handed and return that same one, so a decorator written below one of them applied to direct calls alone: a `@retry(attempts=3)` under `@tasks.every` made one attempt on every scheduled run and three on a direct call, silently. `Retry`, `@retry`, `Fallback`, `@fallback`, `Shield`, `@shield`, `CircuitBreaker`, `Bulkhead`, `Timeout`, `@cached`, `@idempotent`, `@measure`, and `@instrument` now raise `TypeError` at decoration time, where `Stack` and `RateLimiter` already did. Put the registering decorator on top. To wrap a registered function for a direct call, use `Stack.run` or `Shield.run` for an async one, or apply the decorator to a function that calls it, which works for any. `IntervalTask` and `CronTask` gain a read-only `function` property, which is what `Tasks.add_task` marks, so a `Task` of your own is covered when it exposes one too. ([#788](https://github.com/grelinfo/grelmicro/issues/788))

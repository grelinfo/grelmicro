...
* 🔒 `CachedResponses(include=...)` refuses a pattern naming a read behind a security scheme however the pattern names it. A pattern is matched against the URL a request asks for, and a route template stands for many, so `include={"/users/me": 30}` on `GET /users/{uid}` named no template, walked past the refusal, and cached a gated read: the second caller was handed the first caller's response, and a request carrying no credential was answered `200` with it. ([#813](https://github.com/grelinfo/grelmicro/issues/813))

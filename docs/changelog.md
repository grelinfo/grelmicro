...
* 🐛 A `Bulkhead` refuses a `uses=` entry that is not an async context manager, with a `TypeError` naming the bulkhead and the type it got. It used to construct and raise a `TypeError` naming neither, on the first entry. ([#791](https://github.com/grelinfo/grelmicro/issues/791))

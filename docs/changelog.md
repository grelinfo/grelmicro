...
* 🐛 `UvicornFormatter` and `UvicornAccessFormatter` accept `use_colors`. Uvicorn writes it into a log config when it is started with `--use-colors` or `--no-use-colors`, and the formatters raised `ValueError: Unable to configure formatter` instead. The flag turns colors on and off for TEXT and PRETTY, and never colorizes JSON or logfmt. ([#737](https://github.com/grelinfo/grelmicro/issues/737))
...
* 🐛 `color_message` no longer reaches the sink. Uvicorn attaches an ANSI-colored copy of its message under that name, which rendered as escape sequences in a field. ([#737](https://github.com/grelinfo/grelmicro/issues/737))

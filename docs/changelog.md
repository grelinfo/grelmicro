...
* ✨ `dict_config(env_load=...)`, `formatter(env_load=...)` and `handler(env_load=...)` read `GREL_LOG_*` from a process that cannot set `GREL_ENV_LOAD`, the same word `configure()` takes.

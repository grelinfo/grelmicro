...
* 🐛 The OpenAPI annotation for idempotency follows `include=` and `exclude=`. An operation the middleware passes through was described with the idempotency header and the responses only a covered operation can get. An `include` that matches no declared route at all, which is what a mounted app leads to, has what `exclude` leaves described rather than nothing. ([#688](https://github.com/grelinfo/grelmicro/issues/688))

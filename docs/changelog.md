...
* ✨ Litestar handlers declare how they are authenticated: `opt=Anonymous()` serves one without a credential, and `guards=[Authenticated(scopes=[...])]` answers `401` or `403` with the challenge. `Anonymous()` is Litestar's own `exclude_from_auth` key, so a handler written for Litestar's authentication middleware is public too. ([#839](https://github.com/grelinfo/grelmicro/issues/839))
...
* ✨ FastAPI routes declare how they are authenticated: `CurrentPrincipal` and `Claims` hand a handler the verified caller, `Authenticated(scopes=[...])`, built on `fastapi.Security`, answers `403` for a missing scope, and `Anonymous()` serves a route without a credential. ([#839](https://github.com/grelinfo/grelmicro/issues/839))

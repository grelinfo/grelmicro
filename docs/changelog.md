...
### Fixed
* 🐛 `document_idempotency(app)` describes every installed `IdempotencyMiddleware`, each with its own paths and its own two header names. An app running a second set of rules had one of them published. ([#688](https://github.com/grelinfo/grelmicro/issues/688))
...
* ✨ The OpenAPI schema describes the replay header on every response the app answers, beside the key header it already described. The refusals the middleware answers before the app runs never replay, so they are left alone. The name is a service's to pick, so a generated client reads it there. ([#688](https://github.com/grelinfo/grelmicro/issues/688))

...
* 🐛 A `Bulkhead` resolves `uses=` the way `Grelmicro(uses=[...])` does. `Bulkhead("reports", uses=[redis])` opened the provider and overrode nothing, so a `Lock` inside the scope quietly used the app's backend instead of the scoped one. A bare backend now becomes its Component, and a bare Provider gains a default Component for every kind it serves, while two bare Providers fill no defaults and raise `AmbiguousProviderError` when no Component is listed beside them. A Provider listed after the Component that borrows it is moved ahead of it, while a Component in `uses=` is never rebound, because it may be the app's too, and two Components claiming one `(kind, name)` raise `ComponentAlreadyRegisteredError` instead of the second quietly winning. One entry listed twice opens once, a Provider the app already holds is left alone rather than opened and closed a second time, and a Component that opens ahead of the Provider it borrows says which one to move rather than reporting a closed client. One difference stands: the scope never adopts a Provider `uses=` leaves out, because it opens on the app's exit stack and would close a Provider the app still holds. The [backend check](deployment.md#the-backend-check) covers the filled Components too, so a provider that cannot serve the environment is named on the first entry where it used to pass unnoticed. ([#791](https://github.com/grelinfo/grelmicro/issues/791))

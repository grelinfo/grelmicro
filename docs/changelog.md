...
### Fixed
* 🐛 A `functools.partial` or a callable object no longer puts a memory address in a metric name or a cache label. `@measure` named a partial from its `repr`, which carries the address of the function it wraps, so every restart of the process opened a new metric. `@cached(ttl=...)` did the same to its private cache's name. Both now name the function behind the partial. ([#691](https://github.com/grelinfo/grelmicro/issues/691))

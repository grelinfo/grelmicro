...
* 🐛 A `Bulkhead` wraps a bare first-party backend in `uses=` into its Component, as `Grelmicro(uses=[...])` does. `Bulkhead("reports", uses=[MemoryLockAdapter()])` opened the adapter and overrode nothing, so a `Lock` inside the scope resolved no backend. ([#791](https://github.com/grelinfo/grelmicro/issues/791))

...
* 🐛 A `Bulkhead` gives its permit back when the scope it opens on first entry fails. A `uses=` provider that was briefly down took a permit and never returned it, so a bulkhead saturated itself one failed entry at a time. ([#689](https://github.com/grelinfo/grelmicro/issues/689))
...
* ✨ A `Stack` takes one name for one external system: every pattern in it carries that system's name, and a pattern naming another is refused where it is written. Two call sites hitting one dependency then share one circuit and one budget, instead of splitting it into circuits that each have to fail on their own. ([#689](https://github.com/grelinfo/grelmicro/issues/689))

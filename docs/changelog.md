...
* 🐛 The rate limiter backends rejected an unsupported algorithm kind with `AssertionError`, while the circuit breaker backends raised `NotImplementedError`. All of them now raise `NotImplementedError` naming the kind. The Postgres and SQLite adapters also read the provider client before checking the kind, so an unsupported kind needed a live connection to fail.

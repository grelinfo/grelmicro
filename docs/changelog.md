...
* ✨ `dict_config_with(config)` builds the same document from a pre-built `LogConfig`, for settings that never reach the environment. ([#737](https://github.com/grelinfo/grelmicro/issues/737))
* ✨ `formatter()` returns the formatter that renders a record in your format, for naming from a `dictConfig` of your own. ([#737](https://github.com/grelinfo/grelmicro/issues/737))

...
### Fixed
* 🐛 Every grelmicro decorator refuses a function a registrar already holds, instead of wrapping calls that registration will never make. A `@retry(attempts=3)` written under `@tasks.every` made one attempt on every scheduled run and three on a direct call, silently. `@tasks.every`, `@tasks.cron`, `tasks.add_task`, `@health.check`, and `@outbox.handler` all mark what they record, and every decorator that wraps a function now reads that mark. Put the registering decorator on top. ([#788](https://github.com/grelinfo/grelmicro/issues/788))

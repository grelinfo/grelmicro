...
* 🐛 `CircuitBreaker.from_thread` refuses an entry made from the event loop its backend runs on instead of deadlocking on it. It hands the admission to that loop and blocks on the result, which never arrives when the caller is that loop. Reach a breaker from sync code through `asyncio.to_thread(...)`, or use `async with breaker:` from async code. A caller on a different loop is unaffected. The refusal is not an `Exception`, so no `except Exception`, retry, or fallback stands in for a wiring mistake. ([#689](https://github.com/grelinfo/grelmicro/issues/689))

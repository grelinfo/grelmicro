...
* 🐛 Uvicorn's records go through the queue `queue_enabled` installs. Its handlers wrote to `sys.stderr` and `sys.stdout` directly, so a request line was written on the event loop and landed on a different file descriptor than the application's own records. A handler pointed anywhere else keeps its stream, and so does a `FileHandler`. ([#737](https://github.com/grelinfo/grelmicro/issues/737))

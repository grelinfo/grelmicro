...
* ✨ `dict_config(env_load=...)`, `formatter(env_load=...)`, `handler(env_load=...)` and both uvicorn formatters read `GREL_LOG_*` from a process that cannot set `GREL_ENV_LOAD`, the same word `configure()` takes.

...
* ✨ `python -m grelmicro check --app` warns about a path pattern that matches no route. A mistyped one turns a rule off without saying so, and nothing matching is the only trace it leaves. ([#813](https://github.com/grelinfo/grelmicro/issues/813))

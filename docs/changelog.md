...
* 🐛 The OpenAPI annotation for idempotency follows `include=` and `exclude=`. An operation the middleware passes through was described with the idempotency header and the responses only a covered operation can get. An `include` written for a mounted app matches no declared route, and what `exclude` leaves is described rather than nothing, while an `exclude` that empties the selection on its own is followed. ([#688](https://github.com/grelinfo/grelmicro/issues/688))
...
* ✨ The OpenAPI schema describes the replay header on the responses of every covered operation, beside the key header it already described. The name is a service's to pick, so a generated client reads it there. ([#688](https://github.com/grelinfo/grelmicro/issues/688))

...
* 🐛 A `Match` predicate that returns a non-bool is warned about once per predicate, not once per address. The registry held `id(predicate)` with nothing keeping the predicate alive, so a collected one silenced the next predicate that landed on its address and the entries piled up. A predicate that cannot be tracked, an `operator.attrgetter` or an unhashable instance among them, is reported every time rather than raising out of the matcher. ([#782](https://github.com/grelinfo/grelmicro/pull/782))

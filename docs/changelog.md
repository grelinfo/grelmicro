...
### Added
* ✨ `micro.install(app)` refuses a route that requires a caller where it could never get one: one declaring `Anonymous()`, or one in `exclude`. The app is checked again when it starts. ([#839](https://github.com/grelinfo/grelmicro/issues/839))

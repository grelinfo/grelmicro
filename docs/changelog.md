...
* ✨ A header name that is not an HTTP field name raises `SettingsValidationError` when the component is built, instead of reaching the wire as a broken header. `replay_header` also refuses a name the response already carries, such as `Content-Length`. ([#688](https://github.com/grelinfo/grelmicro/issues/688))
* ✨ A replay marks the response under one value. A stored header carrying the replay name is dropped before the marker is added, so a handler setting that name itself no longer leaves two. ([#688](https://github.com/grelinfo/grelmicro/issues/688))

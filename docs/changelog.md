...
* 🐛 A colon inside a path is no longer read as a credential. `https://h//path:x@y` carries no userinfo and is left alone, on a single-host URL and a multi-host DSN alike. A `//` that opens the text, a query value or a fragment still carries one and is still masked. ([#833](https://github.com/grelinfo/grelmicro/issues/833))

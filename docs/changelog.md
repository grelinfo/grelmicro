...
### Breaking
* 💥 `TaskRouter(tasks=[...])` and `Tasks(tasks=[...])` copy the list they are given instead of keeping it, so changing it afterwards no longer changes the router. Each task is recorded the same way `add_task` records one, so the guard covers a task built by hand as well as one added by a decorator. ([#788](https://github.com/grelinfo/grelmicro/issues/788))

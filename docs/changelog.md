...
* 🐛 A `Bulkhead` resolves `uses=` the way `Grelmicro(uses=[...])` does. A bare backend becomes its Component, and a bare Provider gains a default Component for every kind it serves. Two bare Providers fill no defaults, and raise `AmbiguousProviderError` when no Component is listed beside them. `Bulkhead("reports", uses=[redis])` opened the provider and overrode nothing, so a `Lock` inside the scope quietly used the app's backend instead of the scoped one. ([#791](https://github.com/grelinfo/grelmicro/issues/791))

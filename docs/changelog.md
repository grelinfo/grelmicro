...
* ✨ The OpenAPI schema describes the replay header on the responses of every covered operation, beside the key header it already described. The name is a service's to pick, so a generated client reads it there. It is published as a header that may appear, because which responses carry it is decided by the middleware order at runtime. ([#688](https://github.com/grelinfo/grelmicro/issues/688))

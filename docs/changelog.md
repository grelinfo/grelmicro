...
### Added
* ✨ Authentication writes a security event for every refusal and every ban on `grelmicro.security.events`, with Elastic Common Schema categorization, the route template and the client address, repeats from one address written once a minute. `grelmicro.authentication.attempts`, `grelmicro.client_bans.started` and `grelmicro.client_bans.active` count them, and `AuthenticatedRequests(enduser=True)` names the caller on the server span and on a refused token whose signature verified. ([#864](https://github.com/grelinfo/grelmicro/issues/864))
* ✨ `AccessLog(enduser=True)` writes who called as `enduser.id`, the subject of an authenticated caller, so one record says who asked for what and what they got. Off by default, because a subject can be personal data. ([#872](https://github.com/grelinfo/grelmicro/pull/872))
* ✨ `AuthenticatedRequests(resource=...)` publishes the service's protected resource metadata (RFC 9728) and adds `resource_metadata` to every bearer challenge, so a client that discovers authorization at runtime, such as an MCP client, finds the authorization server from the service's URL alone. ([#857](https://github.com/grelinfo/grelmicro/issues/857))
* ✨ `AuthenticatedRequests(check=...)` checks every verified caller before the app sees it, cached tokens and websocket handshakes included. Return `None` to refuse a revoked token with `401` and the new `revoked` reason, or return your own user object for every route to read. ([#858](https://github.com/grelinfo/grelmicro/issues/858))
* ✨ `micro.install(app)` refuses a route that requires a caller where it could never get one: one declaring `Anonymous()`, or one in `exclude`. It also refuses a route requiring a scope that is not an OAuth scope token, such as one a `Security` around `CurrentPrincipal` names. The app is checked again when it starts. ([#839](https://github.com/grelinfo/grelmicro/issues/839))
* ✨ Starlette endpoints require scopes with `@Authenticated(scopes=[...])`, on a function, an `HTTPEndpoint` method or a websocket, answering `401` or `403` with the challenge. ([#839](https://github.com/grelinfo/grelmicro/issues/839))
* ✨ `grelmicro check` reads `AuthenticatedRequests`: each endpoint says `authenticated`, with the scopes it requires, or `anonymous`, and one it authenticates no longer claims a cached or replayed response. An `Anonymous()` route stays cacheable and replayable. ([#839](https://github.com/grelinfo/grelmicro/issues/839))
* ✨ Litestar handlers declare how they are authenticated: `opt=Anonymous()` makes the credential optional on one, and `guards=[Authenticated(scopes=[...])]` answers `401` or `403` with the challenge. `Anonymous()` is Litestar's own `exclude_from_auth` key, so a handler written for Litestar's authentication middleware is public too. ([#839](https://github.com/grelinfo/grelmicro/issues/839))
* ✨ `AuthenticatedRequests` describes itself in the FastAPI and Litestar OpenAPI schema: a bearer token or `openIdConnect` security scheme, required on every covered operation with the scopes its route declares, and the `401`, `403` and `429` it answers. `openapi=False` leaves the schema alone, and `JWTVerifier.metadata_url` names the discovery document a verifier read. ([#839](https://github.com/grelinfo/grelmicro/issues/839))
* ✨ FastAPI routes declare how they are authenticated: `CurrentPrincipal` and `Claims` hand a handler the verified caller, `Authenticated(scopes=[...])`, built on `fastapi.Security`, answers `403` for a missing scope, and `Anonymous()` makes the credential optional on a route, verifying a token that is sent, with `OptionalPrincipal` handing the handler the caller or `None`. ([#839](https://github.com/grelinfo/grelmicro/issues/839))
* ✨ `AuthenticatedRequests(verifier, exclude=...)` authenticates every request and websocket handshake with a bearer token, and puts the verified caller in `request.user` and `request.auth`. `micro.install(app)` places it ahead of every other answering middleware of ours and opens its verifier with the app. A token naming a new key waits for one refresh, `bans=` with `trusted=` refuses a caller that keeps forging, and a refused websocket gets the `401` on a server that can send it. A verifier whose `verify` answers with an awaitable is awaited. `AuthenticatedRequestsMiddleware` is the same for a hand-built stack. ([#839](https://github.com/grelinfo/grelmicro/issues/839))
* ✨ `ErrorResponses` renders authentication refusals with their RFC 6750 `WWW-Authenticate` challenges: `AuthenticationRequiredError` and `TokenRejectedError` answer `401`, `AmbiguousCredentialsError` `400` and `InsufficientScopeError` `403`. `SigningKeysUnavailableError` answers `503`. ([#839](https://github.com/grelinfo/grelmicro/issues/839))
* ✨ `JWTVerifier.discover(issuer, audience=...)` finds the JWKS URL in the issuer's RFC 8414 authorization server metadata or OpenID Connect discovery document, so one URL configures a verifier. The document must name the issuer exactly and an `https` key set, and it is read again once per `ttl`. While the metadata cannot be fetched, the key set it last named is still refreshed. `DiscoveryConfig` is the config it builds. ([#849](https://github.com/grelinfo/grelmicro/issues/849))
...
### Added
* ✨ `JWTVerifier` verifies the bearer token a caller presents, against a fixed key set and claim policy. Signature, `exp`, `nbf`, `aud` and `iss` are checked in a compiled core, so a verification costs about 12 microseconds where a pure-Python library costs 46. Install it with `pip install "grelmicro[jwt]"`, which pulls the compiled `grelmicro-core` wheel. ([#840](https://github.com/grelinfo/grelmicro/issues/840))
* ✨ `JWTConfig.from_jwks()` builds a verifier from the JWKS an OIDC provider publishes, reading the `kid` and `alg` off each key and skipping the ones published for encryption. AWS Cognito and Microsoft Entra ID both work, including Entra keys that carry no `alg`. ([#840](https://github.com/grelinfo/grelmicro/issues/840))
* ✨ `JWKSVerifier` fetches the JWKS itself and follows the provider when it rotates. `refresh()` is a coroutine you schedule and `verify()` stays synchronous, so no request waits on the provider. A token naming a key the verifier does not hold is refused and marks the key set stale, and `retry_interval` stops invented `kid` values from making the service hammer its provider. The endpoint must be `https`, bodies are abandoned past `max_bytes`, redirects are not followed, and a failed refresh leaves the loaded keys in place. ([#840](https://github.com/grelinfo/grelmicro/issues/840))
* ✨ A verified token is cached until its own `exp` or `cache_ttl` passes, whichever comes first, so a client that resends one token is answered in about 320 nanoseconds instead of 12 microseconds. The cache keys on a SHA-256 digest computed in the core, so a process holds no live bearer token beyond the request that presented it. `cache_key="token"` keys on the encoded token and is 94 nanoseconds faster. ([#840](https://github.com/grelinfo/grelmicro/issues/840))
* ✨ `required` covers any claim, not only the ones RFC 7519 registers, so `required=["exp", "tenant"]` refuses a token that carries no tenant. ([#840](https://github.com/grelinfo/grelmicro/issues/840))
* ✨ `ClientBans` refuses a caller that keeps presenting tokens that do not verify, so a forged-token flood costs a dictionary lookup instead of a signature check. It is off unless you pass one to `JWTVerifier(config, bans=...)` or `JWKSVerifier(config, bans=...)`, and a verifier built with bans refuses a call that gives it no `client` rather than quietly counting nothing. Only `signature`, `malformed` and `algorithm` are counted by default, because counting `unknown-key` or `expired` would ban a service's own users whenever the provider rotates its keys. ([#840](https://github.com/grelinfo/grelmicro/issues/840))
* 🔒 `required` only ever adds to what is enforced. Naming a claim used to replace the expiry requirement rather than extend it, so `required=["tenant"]` read as tightening a policy and in fact turned the `exp` check off, and a token carrying no expiry was then accepted for as long as its key stayed published. `exp` is now enforced whether or not it is named, and a claim written as `null` no longer counts as present. ([#840](https://github.com/grelinfo/grelmicro/issues/840))
* 🔒 `JWTClaims.raw` is read-only. A verified claim set is shared by every request presenting that token while it is cached, so writing into it changed what a later request was authorized as. ([#840](https://github.com/grelinfo/grelmicro/issues/840))
* 🔒 Naming an `audience` or an `issuer` requires that claim. A token that omits `aud` no longer satisfies a configured audience, which would have applied the check to the tokens carrying the claim and waved through the ones that did not. Leave `audience` empty for a provider whose tokens carry none, such as an AWS Cognito access token. ([#840](https://github.com/grelinfo/grelmicro/issues/840))

### Internal
* 👷 CI runs only what a change touches. A Python change runs lint, the docs build and the tests. A change under `rust/` runs formatting, clippy and the unit tier against the crate built from it. An unchanged crate is installed from PyPI instead of compiled, which cut that step from 53 seconds to under one. `rust-toolchain.toml` pins the compiler everywhere the core is built. ([#844](https://github.com/grelinfo/grelmicro/issues/844))
* 👷 A release dry run no longer reaches the credentials that publish. Building moved into a job with no environment, and only attesting and publishing stay behind the `pypi` environment, which now accepts a release tag alone and requires an approval. ([#842](https://github.com/grelinfo/grelmicro/issues/842))
* 👷 `grelmicro-core` wheels build for the interpreters pydantic-core ships, with free-threaded builds from 3.14t. The first attempt asked maturin for 3.13t, which it refuses, and produced no wheel on any target. ([#843](https://github.com/grelinfo/grelmicro/issues/843))
* 👷 Dependabot proposes updates to the compiled core's Rust dependencies, which include its crypto provider. ([#845](https://github.com/grelinfo/grelmicro/issues/845))
* 👷 Workflows reference this repository's own actions and reusable workflows with the `$/` syntax, which resolves to the running commit rather than to files on disk. ([#846](https://github.com/grelinfo/grelmicro/issues/846))
...
* 💥 `grelmicro.bulkhead.rejections` is now `grelmicro.bulkhead.calls` with `grelmicro.outcome` set to `admitted` or `rejected`. A rejection count with no denominator could not give a rejection rate. ([#691](https://github.com/grelinfo/grelmicro/issues/691))
* 💥 `grelmicro.rate_limiter.decisions` is now `grelmicro.rate_limiter.calls`, and its outcome reads `admitted` or `rejected` instead of `allowed` or `limited`. Every admission primitive now answers one query. ([#691](https://github.com/grelinfo/grelmicro/issues/691))
* 💥 `grelmicro.retry.attempts` counts attempts. It counted runs, one point per run, whatever the run cost, so a retry storm was invisible. Divide it by the count of `grelmicro.retry.duration` for the amplification. ([#691](https://github.com/grelinfo/grelmicro/issues/691))
* 💥 `grelmicro.circuit_breaker.transitions` carries `grelmicro.circuit_breaker.previous_state` and `grelmicro.circuit_breaker.state` instead of `from` and `to`. The word the gauge uses is now the word the filter uses. ([#691](https://github.com/grelinfo/grelmicro/issues/691))
* 💥 `grelmicro.task.duration` and `grelmicro.retry.duration` carry `grelmicro.outcome`, and `error.type` on a failure, so error latency is separable from success latency on one histogram. ([#691](https://github.com/grelinfo/grelmicro/issues/691))
* 💥 `grelmicro.cache.operations`, `.stale_serves`, `.early_refreshes` and `grelmicro.shield.cache_writes` carry the instance name. Two caches in one process reported as one series. ([#691](https://github.com/grelinfo/grelmicro/issues/691))
* 💥 A `cached(ttl=...)` decorator applied to more than one function builds one private cache per function instead of one shared between them. Each function gets its own `maxsize` budget, and `cache_info()` and `cache_clear()` answer for that function alone. Pass a `TTLCache` to keep sharing one store. ([#691](https://github.com/grelinfo/grelmicro/issues/691))
* 💥 `CachedResponses(include=...)` refuses a pattern written for a path that answers no `GET`. It would cache nothing while reading as though it did. A prefix pattern is unaffected, because a router holds writes beside its reads. ([#813](https://github.com/grelinfo/grelmicro/issues/813))
* 💥 `CachedResponses`, `ConditionalRequests`, `IdempotentRequests`, `RateLimitedRequests` and `AccessLog` refuse a bad value with `SettingsValidationError` instead of `TypeError` or `ValueError`, the one error every setting raises, and the middleware under each of them raises it too. A wrong argument *type* is still a `TypeError` on the middleware, where hand-wired ASGI expects one: a set of paths or methods given as a string. ([#813](https://github.com/grelinfo/grelmicro/issues/813))
* 💥 `CachedResponses(vary_by_query=...)` defaults to `UNSET` rather than `None`. `None` is what the field means by "key on the whole query string", so writing it now says that over any variable, where before it read as an argument nobody passed and the environment answered instead. ([#813](https://github.com/grelinfo/grelmicro/issues/813))
* 💥 `IdempotentRequests(max_body_size=0)` and `ConditionalRequests(max_body_size=0)` are refused. Nothing was held, so every body counted as too large and every request was answered `413`, which is a mistake rather than a setting. `wait_timeout=0` is still accepted and still means a duplicate is answered without waiting. ([#813](https://github.com/grelinfo/grelmicro/issues/813))
* 💥 `AccessLogMiddleware` no longer carries `include`, `exclude`, `quiet`, `query` and `user_agent` as attributes. They are the component's to decide and move with a live reconfigure, so the middleware reads them from the snapshot it is given, the way every other grelmicro middleware already did. ([#813](https://github.com/grelinfo/grelmicro/issues/813))
...
* 💥 The `fastapi` extra needs FastAPI 0.137 or later. From that release `include_router` keeps the router rather than copying its routes into the app, and everything grelmicro reads off a route reads it there. ([#744](https://github.com/grelinfo/grelmicro/issues/744))
* 💥 `configure()` moves uvicorn's own lines from `sys.stderr` to the stream the rest of the process writes to, so one process writes one stream and uvicorn's records go through the queue `queue_enabled` installs instead of being written on the event loop. A shell redirecting `2>` to keep uvicorn's startup and error lines apart no longer catches them. A handler already pointed somewhere else keeps its stream, a `FileHandler` keeps the file it opened, and `uvicorn_enabled=False` turns the whole pass off. ([#737](https://github.com/grelinfo/grelmicro/issues/737))
...
* 💥 `Lock.do_thread_*` and the `ReadWriteLock` mode equivalents take the calling thread's identity instead of its ident. They carry ownership across the event loop for a worker thread and were never the documented way in: use `lock.from_thread`, which is unchanged and passes the identity for you. ([#781](https://github.com/grelinfo/grelmicro/pull/781))
* 💥 Every `Match` argument error is a `ValueError` at construction. `Match.exception_message()` refuses a non-string `contains=`, a `regex=` that is neither a string nor a compiled pattern, an invalid regex, a pattern compiled from bytes, and the missing-or-both `contains=`/`regex=` call. `Match.predicate()` refuses a non-callable. Some of these used to raise `TypeError`, `re.error`, or `AttributeError`, none of which pydantic converts, so they escaped every documented `except` in a validator. A non-string `contains=` and a bytes pattern used to build and fail at match time, where the total matcher would now read them as no match forever. ([#782](https://github.com/grelinfo/grelmicro/pull/782))
* 💥 A shield's `timeout_errors=` refuses a `BaseException`-only type wherever it appears. Passed alone or by name it was already refused, passed inside a tuple it was accepted and then never retried, because the shield only retries `Exception` subclasses. ([#782](https://github.com/grelinfo/grelmicro/pull/782))
* 💥 A shield's `timeout_errors=` takes an exception class, a tuple or list of them, or a fully-qualified name. A set, a `deque`, or a generator used to slip through and is refused now, because the value is classified before pydantic sees it: the type check pydantic runs reads `__class__`, which a lazy proxy raises from. ([#782](https://github.com/grelinfo/grelmicro/pull/782))
...
* 💥 A `Bulkhead` refuses two Components claiming one `(kind, name)` in `uses=` with `ComponentAlreadyRegisteredError`, where the second used to quietly win. A module-level bulkhead raises at import. `Grelmicro(uses=[...])` already refused the same list. ([#791](https://github.com/grelinfo/grelmicro/issues/791))
* 💥 A `Bulkhead`'s `uses=` scope overrides what it lists, where a bare Provider used to override nothing. `micro.fake()` and `micro.override(...)` swap the app's Components and a scope answers first, so neither reaches inside `uses=`: build the bulkhead with the backends a test wants. ([#791](https://github.com/grelinfo/grelmicro/issues/791))
...
### Added
* ✨ `TTLCache(name=...)` names a cache, and every cache metric carries it. `micro.cache.ttl()` inherits the component's name, so name each cache when an app builds more than one. The caches grelmicro builds for you carry one too: `Idempotency` and `CachedResponses` use their registration name, and a private `@cached(ttl=...)` cache uses the function it serves. It is a metric label rather than a configuration address: a cache still reads no environment variable. ([#691](https://github.com/grelinfo/grelmicro/issues/691))
* ✨ `grelmicro.task.schedule.delay` records how late a fire started against the instant it was due, which is queueing on a healthy worker and the full replay age on a fire that came back after a restart. ([#691](https://github.com/grelinfo/grelmicro/issues/691))
* ✨ `grelmicro.task.next_run` is the instant of the next fire, as a Unix timestamp, so `grelmicro_task_next_run_seconds - time()` is the time left and a task that stopped shows as a next run stuck in the past. ([#691](https://github.com/grelinfo/grelmicro/issues/691))
* ✨ `grelmicro.lock.attempts`, `grelmicro.lock.renewals` and `grelmicro.lock.holders` report every distributed lock. `grelmicro.lock.mode` tells an exclusive lock, a task lock and a read or write lease apart. A backend nobody can reach counts as `error`, a lock another worker holds as `unavailable`, and a lease gone before the work finished as `lost`, which is the one to page on. ([#691](https://github.com/grelinfo/grelmicro/issues/691))
* ✨ `grelmicro.leader_election.attempts` and `grelmicro.leader_election.leading` report the election. The gauge reads 1 on the leader and 0 on every standby, so a healthy fleet sums to exactly one and a split brain sums to two. ([#691](https://github.com/grelinfo/grelmicro/issues/691))
* ✨ Every counter declares the semantic-convention unit for what it counts, such as `{run}` or `{call}`, instead of `1`. The Prometheus exporter drops the annotation, so no metric name changes. ([#691](https://github.com/grelinfo/grelmicro/issues/691))
* ✨ A nested mapping in a config file is written both ways, because the document does not say which it is: as JSON under its own name, for a field that takes a mapping such as `include` or `headers`, and walked as a level of nesting as well. Whichever name reaches a field fills it. A key is normalised the way an instance name is, so a `Lock("cart.v2")` is filled by a document that writes the name as it was given. ([#813](https://github.com/grelinfo/grelmicro/issues/813))
* ✨ Live reload tunes what a request costs, never what it is protected by. `CachedResponses`, `RateLimitedRequests` and `AccessLog` are fully live. `ConditionalRequests` and `IdempotentRequests` are configured at startup, every field of them, because every one of theirs protects something: taking a path out of either one, turning off payload fingerprinting, or lowering the body limit each removes a guarantee nobody is told about. A key a file may not change is reported and the rest of the file still applies. ([#813](https://github.com/grelinfo/grelmicro/issues/813))
* ✨ `RateLimitedRequests()` documents the `429` and the `Retry-After` it carries on every operation in the OpenAPI schema, and the `RateLimit` and `RateLimit-Policy` fields on every success beside it, because a client reads what it has left off the answer it got rather than off the refusal it did not, so a generated client has a branch for it. Every operation rather than the metered ones, because what is metered is tuned live and the schema is not. Pass `openapi=False` to leave the schema alone. ([#813](https://github.com/grelinfo/grelmicro/issues/813))
* ✨ The OpenAPI schema is built once, from the app as installed, and nothing it states is live. A published contract is not a tuning knob, and each replica polls its own source on its own clock, so a live schema would have two pods publishing two different documents. ([#813](https://github.com/grelinfo/grelmicro/issues/813))
* ✨ Every HTTP component is tuned live from a mounted ConfigMap. `GREL_CACHED_RESPONSES_TTL`, `GREL_ACCESS_LOG_EXCLUDE` and the rest reach the running middleware without a restart, because it reads a snapshot the component publishes rather than the values it was built with. ([#813](https://github.com/grelinfo/grelmicro/issues/813))
* ✨ `CachedResponsesConfig`, `ConditionalRequestsConfig`, `IdempotentRequestsConfig`, `RateLimitedRequestsConfig` and `AccessLogConfig`, each with a `from_config()` classmethod, for a configuration assembled from a settings tree. ([#813](https://github.com/grelinfo/grelmicro/issues/813))
* ✨ `RateLimitedRequests(include=...)` meters only the paths it names, so all five HTTP components now take the same two selectors. ([#813](https://github.com/grelinfo/grelmicro/issues/813))
* ✨ `CachedResponses(include=("/products/*",))` takes a tuple as well as a mapping, and keeps each path for the component's own `ttl`. ([#813](https://github.com/grelinfo/grelmicro/issues/813))
* ✨ `python -m grelmicro check --app` warns about an `include` pattern that matches no route. A mistyped one turns a rule off without saying so, and nothing matching is the only trace it leaves. ([#813](https://github.com/grelinfo/grelmicro/issues/813))
* 🔒 `CachedResponses(include=...)` names a path that is never cached when the app refuses it, whichever pattern named it: the store itself answers `None` for a read behind a security scheme, so no way of writing a pattern can put one in the cache. It is also refused where it is written. A pattern is matched against the URL a request asks for, and a route template stands for many, so `include={"/users/me": 30}` on `GET /users/{uid}` named no template, walked past the refusal, and cached a gated read: the second caller was handed the first caller's response, and a request carrying no credential was answered `200` with it. ([#813](https://github.com/grelinfo/grelmicro/issues/813))
* ✨ A row says `(some paths)` where a pattern names one URL of a route that serves many, such as `exclude=("/users/me",)` on `GET /users/{uid}`. Saying the rule applies to the endpoint would overstate it and saying it does not would be wrong. ([#813](https://github.com/grelinfo/grelmicro/issues/813))
* ✨ `micro.describe(app).endpoints` says what every registered component does to each route, so "what happens to `GET /products`" is one line instead of five registrations. `python -m grelmicro check app:micro --app app:app` prints it. ([#813](https://github.com/grelinfo/grelmicro/issues/813))
* ✨ A mounted file decodes a field holding many values the way the environment does, so one ConfigMap says the same thing whether it is mounted as a volume or read through `envFrom`. A field whose value is `Optional` was skipped before, which dropped every other key in the same file with it. ([#813](https://github.com/grelinfo/grelmicro/issues/813))
* ✨ A malformed value on a field holding many values says that JSON is what it takes, instead of naming the source that failed to parse it. ([#813](https://github.com/grelinfo/grelmicro/issues/813))
* ✨ `RateLimitedRequests(limiter, trusted=...)` turns a caller over its budget away at the edge, so a service stops writing the `429` by hand. A request spends one token of every limiter listed, so a burst limit stands beside a daily one. ([#739](https://github.com/grelinfo/grelmicro/issues/739))
* ✨ Allowed or refused, the response states what the caller has left in the `RateLimit` and `RateLimit-Policy` fields of the RateLimit header specification, and a refusal adds `Retry-After` and travels the same `AdmissionError` path every other rejection takes. `legacy_headers=True` adds the superseded `X-RateLimit-*` fields. ([#739](https://github.com/grelinfo/grelmicro/issues/739))
* ✨ The bucket is the client address a trusted proxy vouched for, resolved once per request and shared with everything downstream. A limiter keyed on the socket peer meters the ingress rather than the caller, so `trusted=` or a `key=` of your own is required. ([#739](https://github.com/grelinfo/grelmicro/issues/739))
* ✨ `RateLimited(limiter, cost=...)` meters one FastAPI route on top of whatever the app meters, for an endpoint that costs more than the rest. ([#739](https://github.com/grelinfo/grelmicro/issues/739))
* ✨ A rejection carrying `headers` puts them on the answer, so what a refusal has to tell the client travels with the status. Starlette, FastAPI and Litestar all merge them. ([#739](https://github.com/grelinfo/grelmicro/issues/739))
* ✨ `CachedResponses()` answers a repeated read from the cache instead of the handler. Declare `dependencies=[CachedResponse(ttl=60)]` on a route, or name the URLs with `include={"/products/*": 60}` on any ASGI framework. It rides the registered `Cache`, so a response one replica computed answers the callers of every other one, and one cold key runs the handler once rather than once per caller. ([#744](https://github.com/grelinfo/grelmicro/issues/744))
* ✨ A stored response carries an `ETag` and an `Age`, and a client that already holds the tag is answered `304 Not Modified` with no body. ([#744](https://github.com/grelinfo/grelmicro/issues/744))
* ✨ A response is never stored when it was answered for one caller: a request carrying `Authorization` or `Cookie`, a response carrying `Set-Cookie` or a `Cache-Control` refusing it, anything but a `200`, and a `Vary` naming a header outside `vary_by_headers`. None of those is configurable, because each one is the same mistake. ([#744](https://github.com/grelinfo/grelmicro/issues/744))
* ✨ `CachedResponses.purge()` drops every response it stored, for a write that changes what a read answers. ([#744](https://github.com/grelinfo/grelmicro/issues/744))
* ✨ `dict_config()` returns a logging configuration the application server applies itself, so the process reads in one format from its first record. Uvicorn takes it as `log_config`, Gunicorn and Hypercorn as `logconfig_dict`, Granian as `log_dictconfig`. It covers the two cases `configure()` cannot reach: the parent process behind `--reload` and `--workers`, which never imports your application, and a script that calls `configure()` and then `uvicorn.run(app)`. ([#737](https://github.com/grelinfo/grelmicro/issues/737))
* ✨ `dict_config(env_load=...)`, `formatter(env_load=...)`, `handler(env_load=...)` and both uvicorn formatters read `GREL_LOG_*` from a process that cannot set `GREL_ENV_LOAD`, the same word `configure()` takes. ([#737](https://github.com/grelinfo/grelmicro/issues/737))
* ✨ `dict_config_with(config)` builds the same document from a pre-built `LogConfig`, for settings that never reach the environment. Both documents are a snapshot of the settings they were built from, and JSON-serializable, so one is written to the file `uvicorn --log-config` reads. ([#737](https://github.com/grelinfo/grelmicro/issues/737))
* ✨ `formatter()` and `handler()` return the pieces the document is made of, for naming from a `dictConfig` of your own. `handler()` writes to the stream the rest of the process writes to, and starts the queue writer when the settings ask for one and none is running. ([#737](https://github.com/grelinfo/grelmicro/issues/737))
...
* ✨ A middleware grelmicro installs says whether it answers a request or only watches one. A watcher wraps outside the middleware the app added itself, so an access log records the request an outer layer refused, with the status it was refused with. One that may answer, an idempotent replay above all, stays innermost where it can never be the reason a request skipped authentication. ([#742](https://github.com/grelinfo/grelmicro/issues/742))
* ✨ `AccessLog()` writes one structured record per HTTP request, carrying the caller `grelmicro.security` resolved rather than the socket peer, the route template, the duration, and the trace context, under OpenTelemetry semantic convention field names. The level follows the answer: `5xx` an error, `4xx` a warning, and a probe that answered a debug, so Kubernetes polling every few seconds stops crowding out the requests a person wanted to read. Registering it silences uvicorn's own access log. ([#742](https://github.com/grelinfo/grelmicro/issues/742))
* ✨ `configure(queue_enabled=True)` hands log writes to a background thread, so a slow sink stops showing up as request latency. The record is still rendered on the calling thread, so the trace context and every bound field survive. One writer serves all three backends. ([#743](https://github.com/grelinfo/grelmicro/issues/743))
* ✨ The log queue is off by default, because it earns its keep only on a slow sink. Against a no-op write it costs about 0.9 µs per record. A burst of 5000 records against a sink taking 200 µs a line fits in the queue and takes the caller from 276 µs to under 8 µs a record, with all 5000 written. Sustained load past what the sink can drain drops about a quarter of them instead, reported as it goes, which is the trade the bound buys. Run `python benchmarks/log_queue_benchmark.py` for the table. ([#743](https://github.com/grelinfo/grelmicro/issues/743))
* ✨ `GREL_LOG_QUEUE_SIZE` sets how many rendered records the queue holds, 10000 by default. A full queue drops the arriving record rather than blocking the caller, and reports the count as one `ERROR` on `grelmicro.log` once it has room again. ([#743](https://github.com/grelinfo/grelmicro/issues/743))
...
* ✨ `PostgresProvider.from_engine(engine)` borrows a SQLAlchemy `AsyncEngine` instead of opening a second pool. Every operation takes a connection from the engine and unwraps it to asyncpg, so the statements run unchanged and the database sees one pool. The engine has to be `postgresql+asyncpg`, and an `AsyncSession` or an `AsyncConnection` is refused: those carry the transaction the caller has open, and a lock released inside it would come back locked on a rollback. A borrowed connection is cleaned before it goes back, so an open transaction or a `LISTEN` of grelmicro's is never found by the application's next query, and one that cannot be cleaned is dropped rather than pooled. The application's own session settings are left alone, because the engine applies those once per connection and never again. Nothing is sent when there is nothing to undo, so a statement run through the provider costs one round trip, not two. ([#685](https://github.com/grelinfo/grelmicro/issues/685))
* ✨ `EventLoopDeadlockError` says a sync entry point was reached from the event loop that has to serve it. A `BaseException`, so `except Exception`, a `Retry`, and a `Fallback` all pass it through: a wiring mistake no retry can fix reads as a failure, never as a fallback value. It is the one grelmicro error that is not an `Exception`. ([#787](https://github.com/grelinfo/grelmicro/issues/787))
* ✨ `Stack("recs", patterns=[...])` applies resilience patterns in the safe order, `Fallback → Retry → CircuitBreaker → RateLimiter → Bulkhead → Timeout → call`, whatever order you list them in. It takes built patterns and adds no configuration of its own, decorates and offers `run(fn, ...)`, skips a `None` entry the way `Grelmicro(uses=[...])` does, and refuses two of one pattern or a stack of none. ([#689](https://github.com/grelinfo/grelmicro/issues/689))
* ✨ Inside a `Stack`, a refusal by its own patterns is read as a call that never happened: its `CircuitBreakerError` is never retried by its `Retry`, and its `RateLimitExceededError` and `BulkheadFullError` are recorded as no outcome by its `CircuitBreaker`. Every other error reaches each pattern exactly as its own `when=` or `ignore_exceptions` decides. ([#689](https://github.com/grelinfo/grelmicro/issues/689))
* ✨ `RateLimiter` decorates. `@limiter` meters a function under the `default` bucket, and `@limiter(key="user:{user_id}")` meters per argument with the same key vocabulary `@cached` uses, plus `key_maker=`, `cost=`, and `max_wait=` for a bounded wait instead of an immediate refusal. ([#689](https://github.com/grelinfo/grelmicro/issues/689))
* ✨ A `@limiter` applied under a task decorator is refused, for the same reason a `Stack` is: the schedule holds the function it was handed, so a limiter below it would meter direct calls and leave every scheduled run unmetered. ([#689](https://github.com/grelinfo/grelmicro/issues/689))
* ✨ A `Stack` applied under a task decorator is refused. A task decorator registers the function it is handed and returns it unchanged, so a `Stack` below it wrapped direct calls only and left every scheduled run unprotected, silently. Put the task decorator on top. ([#689](https://github.com/grelinfo/grelmicro/issues/689))
* ✨ Export `Pattern`, the type of everything `Stack(patterns=[...])` accepts, and `RateLimiterBinding`, what `limiter(key=...)` returns. ([#689](https://github.com/grelinfo/grelmicro/issues/689))
* 📝 [Composing patterns](resilience/composition.md) says which name a pattern takes: `CircuitBreaker` and `RateLimiter` are named for the external system and shared between call sites, while `Retry`, `Timeout`, and `Fallback` are named for the call. One system usually has several stacks that share one breaker instance. ([#689](https://github.com/grelinfo/grelmicro/issues/689))
* ✨ Register `ErrorResponses()` and every rejection grelmicro raises answers the client as an [RFC 9457](https://www.rfc-editor.org/rfc/rfc9457.html) problem detail. `micro.install(app)` wires the handler on FastAPI, Starlette, and Litestar, so a rate limiter over budget, a full bulkhead, an open circuit breaker, an elapsed deadline, or an idempotency conflict returns `application/problem+json` instead of becoming a `500`. Leave the component out and grelmicro changes nothing about how your framework answers an error. ([#740](https://github.com/grelinfo/grelmicro/issues/740))
* ✨ `grelmicro.http` ships `ProblemDetail` and `TMFError`, the Pydantic models for each body, `RenderedError` for what an integration writes, `send_error(send, rendered)` to write one from a pure-ASGI middleware, and `merge_headers` for an integration building a framework response. ([#740](https://github.com/grelinfo/grelmicro/issues/740))
...
* ✨ `grelmicro.integrations.starlette` is the home of the pure-ASGI wiring: the lifespan, the per-request binding, the error responses, and `error_response`. A Starlette app no longer imports from a module named after another framework. ([#775](https://github.com/grelinfo/grelmicro/issues/775))
* ✨ `ConditionalRequests()` answers conditional requests, so a write cannot erase a change it never saw. `micro.install(app)` puts an `ETag` on responses and answers `304` to a read that already holds one, and `check_precondition(etag_of(cart.version))` refuses a stale write with `412` and an unconditional one with `428`. Works on FastAPI, Starlette, and Litestar. ([#771](https://github.com/grelinfo/grelmicro/issues/771))
* 🐛 A `4xx` or `5xx` is never answered as `304`. A read that recorded a version and then failed, because the row was deleted or the caller was refused, would have told a client its cached copy was still good. ([#771](https://github.com/grelinfo/grelmicro/issues/771))
* 🐛 A handler that sets its own `ETag` gets the `304` shortcut, which used to be the one case that lost it. ([#771](https://github.com/grelinfo/grelmicro/issues/771))
* 🐛 A background task no longer delays the response. The middleware decides when the body completes rather than when the app returns, and a framework runs background work inside that call. ([#771](https://github.com/grelinfo/grelmicro/issues/771))
* 🐛 On Litestar, the middleware lands under every layer the app built, not only the first. With CORS or a plugin configured, one hop left it above the layer that renders exceptions, and the framework's `500` was stored and replayed. ([#771](https://github.com/grelinfo/grelmicro/issues/771))
* 🐛 `include=("/payments/*",)` selects `/payments` as well as what sits under it, so naming a router's prefix covers the create route on the router itself. ([#687](https://github.com/grelinfo/grelmicro/issues/687))
* 🐛 A second `micro.install(app)` wires nothing twice. It used to add a second binding on Starlette and FastAPI, and a second copy of every registered middleware on Litestar, which stored, captured and answered twice. ([#687](https://github.com/grelinfo/grelmicro/issues/687))
* 🐛 A streamed response carries the entity tag a handler recorded with `check_freshness`, and is still swapped for a `304` when the client already holds it. The tag is known before the first byte, so neither needs the body. ([#771](https://github.com/grelinfo/grelmicro/issues/771))
* 💥 An `Idempotency-Key` holding a byte outside printable ASCII is refused with `400`, and the OpenAPI schema publishes the same rule as a pattern. A control byte or a high byte reached the cache as a key and travelled through proxies that may rewrite it, while the schema already said ASCII. ([#687](https://github.com/grelinfo/grelmicro/issues/687))
* 🐛 A `304` a handler answers itself carries the entity tag it recorded with `check_freshness`, which [RFC 9110](https://www.rfc-editor.org/rfc/rfc9110.html) asks for. ([#771](https://github.com/grelinfo/grelmicro/issues/771))
* 🐛 `install_middleware` refuses an app that already built its middleware stack, the way `add_middleware` does, rather than editing a list nothing reads any more. ([#687](https://github.com/grelinfo/grelmicro/issues/687))
* 🐛 A streamed response is never held back. `ConditionalRequestsMiddleware` buffers a body only while it arrives in one piece, so an event stream reaches the client as it is produced rather than as one message at the end. A streamed response carries no generated entity tag, and neither does a `204`, whose empty body would otherwise hash to one tag for every empty resource. ([#771](https://github.com/grelinfo/grelmicro/issues/771))
* 🐛 On Litestar, `IdempotencyMiddleware` runs under what turns an exception into a response, so an unhandled error is no longer stored and replayed as a sticky `500` for the whole window. It now matches Starlette and FastAPI, where the exception never reaches the middleware. ([#771](https://github.com/grelinfo/grelmicro/issues/771))
* 🐛 `etag_of` refuses a version holding a comma, which separates two entity tags in one header and would have made every `If-Match` miss forever. ([#771](https://github.com/grelinfo/grelmicro/issues/771))
* 🐛 A middleware the app added by hand is not added a second time by `install`, on Starlette and FastAPI as it already was on Litestar. ([#687](https://github.com/grelinfo/grelmicro/issues/687))
* 🔒 `etag_of` and the idempotency `key_maker` check never let a caller's object raise out of them. A lazy proxy raises from `__class__`, a metaclass can raise from `__name__`, and a `model_dump` property runs caller code, so every shape test and every name goes through the same guards the matchers use. A rejected value is named by its type and never printed. ([#771](https://github.com/grelinfo/grelmicro/issues/771))
* 🔒 `micro.install(app)` places grelmicro's middleware innermost, behind every middleware the app added itself. A middleware that answers a request on its own, such as an idempotent replay, can no longer answer before the app's authentication runs. Registering the component after the app's own middleware, which is the natural order, used to put it in front. ([#771](https://github.com/grelinfo/grelmicro/issues/771))
* 🔒 A new `middleware-placement` diagnostic warns on Litestar, which builds its middleware stack at construction so `install` can only wrap the whole app. It names the fix, and a middleware the app already wired at construction is left alone rather than wrapped twice. ([#771](https://github.com/grelinfo/grelmicro/issues/771))
* ✨ `ConditionalRequired` declares the precondition on the route, so the schema marks `If-Match` required exactly where the service refuses without it, and a missing one answers `428` rather than a validation error. ([#771](https://github.com/grelinfo/grelmicro/issues/771))
* ✨ `Conditional` injects the same guards on FastAPI, so a handler writes `conditional.check(cart.version)` with the dependency the framework expects. One implementation underneath, and the imported functions stay the portable form. ([#771](https://github.com/grelinfo/grelmicro/issues/771))
* ✨ Every HTTP middleware selects paths the same way: `include=` names what it acts on, empty meaning everything, and `exclude=` carves out, winning over `include`. A pattern ending with `*` matches as a prefix, so `include=("/payments/*",)` selects a router by its prefix. The OpenAPI annotations follow the same selection. ([#687](https://github.com/grelinfo/grelmicro/issues/687))
* 💥 `IdempotentRequests` takes `ttl=`, `namespace=` and `cache=` instead of an `Idempotency` instance, so the common case reads `IdempotentRequests(ttl=3600)`. The component still owns one and exposes it as `.idempotency` for code that needs the store itself. ([#687](https://github.com/grelinfo/grelmicro/issues/687))
* ✨ `check_freshness(version)` puts the `ETag` on a read and returns whether the client already holds that version, so a handler needs no `Response` object and can skip the work behind a `304`. ([#771](https://github.com/grelinfo/grelmicro/issues/771))
* ✨ A registered HTTP component answers its own rejections. `ConditionalRequests()` alone returns `412` and `428` instead of a `500`, and `IdempotentRequests()` does the same for a conflict raised by the block form. `ErrorResponses()` chooses the format they render in, RFC 9457 by default. ([#771](https://github.com/grelinfo/grelmicro/issues/771))
* ✨ `micro.install(app)` describes the conditional headers in the OpenAPI schema, so Swagger offers `If-Match` on writes and `If-None-Match` on reads, with the `304`, `412` and `428` responses beside them. The header is marked required exactly when `require_precondition` refuses without it. ([#771](https://github.com/grelinfo/grelmicro/issues/771))
* ✨ `etag_of(value)` builds an entity tag from a version token (`int`, `str`, `UUID`, `datetime`) or by hashing a representation (`bytes`, JSON data, a pydantic model) with SHA-256. The serialization is canonical and never the JSON library that happens to be installed, so every replica issues the same tag for the same value. ([#771](https://github.com/grelinfo/grelmicro/issues/771))
* ✨ `PreconditionFailedError` and `PreconditionRequiredError` join the rendered rejections as `412` and `428`. Raise the first yourself when a conditional `UPDATE` changes no row, and the client reads the same answer as a stale `If-Match`. ([#771](https://github.com/grelinfo/grelmicro/issues/771))
* 📝 A [Conditional Requests](http/conditional.md) guide, including the three ways to back the write in a database: a conditional `UPDATE` on a version column, `SELECT ... FOR UPDATE`, or a distributed `ReadWriteLock`, with the first named as the one to reach for. ([#771](https://github.com/grelinfo/grelmicro/issues/771))
* ✨ `IdempotentRequests()` in `uses=[...]` has `micro.install(app)` add `IdempotencyMiddleware` and describe it in the OpenAPI schema, so the container holds the whole wiring. It takes every middleware option, and `openapi=False` leaves the schema alone. ([#687](https://github.com/grelinfo/grelmicro/issues/687))
* ✨ An integration may carry `install_middleware(app, components)`, and a component may carry `asgi_middleware()`. `install` feature-detects both, so a component that asks for a middleware is wired on every framework that takes one and skipped on a framework that serves no HTTP. ([#687](https://github.com/grelinfo/grelmicro/issues/687))
* ✨ Every pattern works on every framework, and a test holds it. `tests/test_framework_parity.py` walks the package for ambient resolution, refuses an empty scan, and runs each pattern inside a FastAPI, Starlette, and Litestar handler and inside a FastStream subscriber. A rejection and an idempotent replay are compared byte for byte across the three HTTP frameworks. ([#771](https://github.com/grelinfo/grelmicro/issues/771))
* 📝 The [Frameworks](frameworks.md) page states what is portable and names the three pieces that are not: `health_router()`, `metrics_router()`, and `document_idempotency(app)`, all of which need something only FastAPI builds. ([#771](https://github.com/grelinfo/grelmicro/issues/771))
...
* ✨ A header name that is not a string, or not an HTTP field name, raises `SettingsValidationError` when the component is built, instead of reaching the wire as a broken header. `replay_header` also refuses a name that directs the client, such as `Content-Type` or `Content-Length`, and `key_header` refuses a name every request already carries, such as `Authorization`, which would key one stored response for every caller that shares the value. ([#688](https://github.com/grelinfo/grelmicro/issues/688))
* ✨ The replay marker means a replay and nothing else. A handler that sets the replay header itself loses it, on a fresh response as well as a replayed one, and the loss is logged once, so a client reading the header's presence is never told a fresh response is a replay. A marker from a second `IdempotencyMiddleware` running under the first is the one exception, and it travels. ([#688](https://github.com/grelinfo/grelmicro/issues/688))
* ✨ The OpenAPI schema describes the replay header on the responses of every covered operation, beside the key header it already described. The name is a service's to pick, so a generated client reads it there. It is published as a header that may appear, because which responses carry it is decided by the middleware order at runtime. ([#801](https://github.com/grelinfo/grelmicro/pull/801))
...
### Fixed
* 🐛 A query parameter that addresses a row rather than holding a secret keeps its value in a log. `sort_key`, `partition_key`, `routing_key`, `cache_key` and `shard_key` read as credentials to the `*_key` rule and never carry one, in any spelling: `sortKey`, `sort-key` and `sort.key` all read the same. Every other name ending in `key` is still masked, `idempotency_key` included, because a leaked one can replay a request. ([#833](https://github.com/grelinfo/grelmicro/issues/833))
* 🐛 A leaked loguru patcher no longer survives a reconfigure. Moving from a format that needs one, such as `{extra[serialized]}`, to a format that does not left the old patcher serializing every record, because loguru replaces a patcher only when it is handed one. ([#833](https://github.com/grelinfo/grelmicro/issues/833))
...
* 🐛 `CircuitBreaker` admits an async callable object on the event loop instead of raising that it cannot be used from there, and `@measure` and `@instrument` time the body rather than the construction of its coroutine. A measured body that raised was never recorded as an error. ([#820](https://github.com/grelinfo/grelmicro/pull/820))
* 🐛 `@measure` no longer raises from inside the call it watches. An instrument name has to start with a letter and carry no brackets, so a function defined in the entry-point script (`__main__.charge`) or a lambda (`<lambda>`) made the OpenTelemetry SDK raise on the first call, and only once metrics were turned on. An app that ran in development failed in production. A name the specification already accepts is untouched, so no metric is renamed. ([#691](https://github.com/grelinfo/grelmicro/issues/691))
* 🐛 A `functools.partial` or a callable object no longer puts a memory address in a name grelmicro derives from it. A `repr` carries the address of the object, so the name changed on every restart of the process. It reached the `@measure` metric name, the `@cached` cache key and its private cache name, the `@shield` name, and the `@instrument` span name. Each now names the function behind the partial, and falls back to the type's qualified name for a callable object. ([#691](https://github.com/grelinfo/grelmicro/issues/691))
* 🐛 The default `@cached` key for a `functools.partial` was unreachable after a restart and from every other replica, because the address in it changed each run. A cache with no explicit `key=` served a permanent miss and grew its keyspace forever. Keys for a named function are unchanged. ([#691](https://github.com/grelinfo/grelmicro/issues/691))
* 🐛 A `@shield` on a partial produced a `GREL_SHIELD_*` environment prefix carrying that address, so the variable an operator had to set could not be predicted, let alone bound. ([#691](https://github.com/grelinfo/grelmicro/issues/691))
* 🐛 Four `type` aliases named something imported only for a type checker, so reading the alias raised `NameError`. A lazily-evaluated alias passes every import and every checker and then fails wherever pydantic, `typing.get_type_hints` or `fast_depends` reads it. `tests/test_public_api.py` now evaluates every alias in the package. ([#691](https://github.com/grelinfo/grelmicro/issues/691))
* 🐛 `ConditionalRequired` on a route of an included router documents `If-Match` as required again. FastAPI 0.137 stopped copying those routes into the app, so nothing found them. ([#744](https://github.com/grelinfo/grelmicro/issues/744))
* 🐛 `GREL_LOG_LEVEL` governs the application server's own lines. Gunicorn and Hypercorn pin a level on their loggers before they apply a log config, so the process answered to two thresholds: one for the application and the server's own for the server. Uvicorn sets its level afterwards, so `--log-level` still wins where it is passed. ([#737](https://github.com/grelinfo/grelmicro/issues/737))
* 🐛 The level is colored in the TEXT format. It was padded to eight characters before being looked up in the color table, so it never matched and only the time and the logger were colored. ([#737](https://github.com/grelinfo/grelmicro/issues/737))
* 🐛 `UvicornFormatter` and `UvicornAccessFormatter` accept `use_colors`. Uvicorn writes it into a log config it was handed as a dict, as `uvicorn.run(app, log_config=...)` does, when it is started with `--use-colors` or `--no-use-colors`. The formatters raised `ValueError: Unable to configure formatter` instead. A log config read from a file never carried the value, so it was never affected. The flag turns colors on and off for TEXT and PRETTY, and never colorizes JSON or logfmt. ([#737](https://github.com/grelinfo/grelmicro/issues/737))
* 🐛 `caller` follows `caller_enabled` on uvicorn's own records, like every other logger's. `UvicornFormatter` forced it off, so the two ways of rendering a uvicorn record disagreed. ([#737](https://github.com/grelinfo/grelmicro/issues/737))
* 🐛 A handler on the log queue follows when the writer is replaced or taken out, whether or not the uvicorn takeover is on. `Log` stopped the writer on exit and gave the root logger back, and a `configure(uvicorn_enabled=False)` after a document replaced it, and in both cases uvicorn's handlers kept the stopped one and wrote inline. ([#737](https://github.com/grelinfo/grelmicro/issues/737))
* 🐛 `asctime` and `color_message` no longer reach the sink. Uvicorn attaches an ANSI-colored copy of its message under `color_message`, and `asctime` is a time a `logging.Formatter` already rendered, which every record carries as `time`. Both rendered as an extra field. ([#737](https://github.com/grelinfo/grelmicro/issues/737))
* 🐛 `include=`, `exclude=` and `quiet=` refuse a bare string. `exclude="/internal/*"` is a sequence of characters, so it was walked one at a time, and the trailing `*` matched every path as a prefix: the middleware then acted on nothing at all, silently. Affects `AccessLog`, `IdempotentRequests`, `ConditionalRequests`, and the three middlewares behind them. ([#742](https://github.com/grelinfo/grelmicro/issues/742))
* 🐛 `Log(uvicorn_enabled=...)` is accepted. `LogConfig` carried the knob and `configure()` took it, but the component did not, so the only way to set it through `Log` was `from_config`. ([#743](https://github.com/grelinfo/grelmicro/issues/743))
* 🐛 A record written through the standard library reaches the format the app configured on every backend. `loguru` and `structlog` installed no root handler, so grelmicro's own component records, and every dependency logging through `logging`, fell through to `logging.lastResort`: the bare message on stderr at warning and above, and nothing at all below it. Those two backends now take the root logger the way the `stdlib` one always has, so a handler your code attached to the root before configuring is replaced rather than kept: attach it after, or on a logger of your own. ([#806](https://github.com/grelinfo/grelmicro/issues/806))
* 🐛 A loguru format template is read as the format it renders, by every writer in the process. `"{extra[serialized]}"` is JSON and `"{extra[logfmt_serialized]}"` is logfmt, wherever the record comes from. Any other template is loguru's alone, so records from the standard library and from uvicorn are written in grelmicro's text format: `UvicornFormatter` rendered those as JSON before, whatever the template said. ([#806](https://github.com/grelinfo/grelmicro/issues/806))
* 🐛 `/metrics` answers `Cache-Control: no-store`, as the health endpoints already did. A caching proxy could serve a stale exposition to a scrape. ([#690](https://github.com/grelinfo/grelmicro/issues/690))
...
* 🐛 `include=` and `exclude=` match the route, not the prefix a mount or a proxy adds. A pattern is now the same in an app served at the root, mounted under another app, or behind a `root_path`, and the same as the path the OpenAPI schema publishes. Only a whole segment is a prefix, so a `root_path` of `/api` leaves `/apikeys` alone. Affects `IdempotencyMiddleware` and `ConditionalRequestsMiddleware`. ([#801](https://github.com/grelinfo/grelmicro/pull/801))
* 🐛 The OpenAPI annotation for idempotency follows `include=` and `exclude=`. An operation the middleware passes through was described with the idempotency header and the responses only a covered operation can get. ([#801](https://github.com/grelinfo/grelmicro/pull/801))
...
* 🐛 A `Bulkhead` resolves `uses=` almost the way `Grelmicro(uses=[...])` does, so `Bulkhead("reports", uses=[redis])` scopes what redis serves instead of overriding nothing. A bare backend becomes its Component, a bare Provider fills a default Component per kind, two bare Providers raise `AmbiguousProviderError`, and a Provider or bare backend this app run already opened is never opened again. One thing listed twice is one item, where the app refuses the list. ([#791](https://github.com/grelinfo/grelmicro/issues/791))
* 🐛 A `Bulkhead` refuses a `uses=` entry that is not an async context manager, with a `TypeError` naming the bulkhead and the type it got. It used to construct and fail on the first entry, with a `TypeError` that named the type but not the bulkhead. ([#791](https://github.com/grelinfo/grelmicro/issues/791))
* 🐛 A `Bulkhead` opens its `uses=` scope once per app run, not once per process. The record lived on the bulkhead and was never released, so a second `Grelmicro` app in the same process entered nothing and ran every call against providers closed with the first, and an entry that had failed part way left the next app resuming at the item that failed. Entering a scope whose app run has gone now raises instead of using closed providers, and only the run that owns a scope enters its items, so a run overlapping it borrows them rather than opening a second set. ([#791](https://github.com/grelinfo/grelmicro/issues/791))
* 🐛 A `Bulkhead` gives its permit back when the scope it opens on first entry fails, and the retry resumes where that entry stopped. A `uses=` provider that was briefly down took a permit and never returned it, so a bulkhead saturated itself one failed entry at a time, and the next attempt entered every item it had already opened a second time. ([#689](https://github.com/grelinfo/grelmicro/issues/689))
* 🐛 Every sync entry point refuses a call made from the event loop its backend runs on instead of deadlocking on it. `lock.from_thread`, both `rwlock` views, `task_lock.from_thread`, `breaker.from_thread`, and the sync `@cached` wrapper hand their work to that loop and block on the answer, which never arrives when the caller is that loop: the process simply stopped making progress, with nothing logged. Each one now raises `EventLoopDeadlockError` and names the way in. A caller on a different loop, or on a thread with no loop at all, is unaffected. ([#787](https://github.com/grelinfo/grelmicro/issues/787))
...
* 🐛 A config value that cannot be classified is refused with the error the component documents. `Retry(when=...)`, `Fallback(when=...)` and a shield's `timeout_errors=` all tested the shape with `isinstance`, which reads `__class__`, so a lazy proxy such as an unbound `werkzeug.local.LocalProxy` raised from inside the validator. Only `ValueError` is converted, so what it raised escaped `except SettingsValidationError` entirely. ([#782](https://github.com/grelinfo/grelmicro/pull/782))
* 🐛 A container passed to `when=` or to a shield's `timeout_errors=` is walked once, and a container that refuses to be walked is an argument error. `__iter__` is caller code, and a lazily-populated container raises from it once it is detached from what filled it. ([#782](https://github.com/grelinfo/grelmicro/pull/782))
* 🐛 `Match | other` and `Match & other` raise `TypeError` when the other side is not a `Match`, instead of building a combination that fails later. `TypeError` here and `ValueError` everywhere else is Python's operator protocol: the operators return `NotImplemented` and the interpreter raises, which is what lets the other side answer with `__ror__` if it can. ([#782](https://github.com/grelinfo/grelmicro/pull/782))
* 🐛 Reporting a broken predicate never raises either. A logging filter is caller code and `Logger.handle` runs filters unguarded, so a request-context filter raising outside a request escaped from the warning itself. ([#782](https://github.com/grelinfo/grelmicro/pull/782))
* 🐛 A warning names a predicate, it never prints one. A `functools.partial` predicate carries the arguments bound into it and an object predicate carries its attributes, so an API key held by either used to reach the log. A literal still reads as itself. ([#782](https://github.com/grelinfo/grelmicro/pull/782))
* 🐛 A `Match.not_*` matcher reports a broken predicate under the name the caller wrote. It delegates to its positive twin, and the twin is what catches the predicate, so `not_exception(check)` said `exception(check)`. ([#782](https://github.com/grelinfo/grelmicro/pull/782))
* 🐛 A `functools.partial` predicate reads as the function it wraps, so two partials in one policy are tellable apart. The arguments bound into it, where an API key would sit, stay out of the label. ([#782](https://github.com/grelinfo/grelmicro/pull/782))
* 🐛 `Match.predicate()` coerces its return the way `Match.exception()` and `Match.result()` do, so a predicate returning `1` engages the strategy and is warned about once instead of returning `1` to it. ([#782](https://github.com/grelinfo/grelmicro/pull/782))
...
### Security
* 🔒 An idempotency key can no longer replay a private response across callers when no identity-aware `key_maker` is configured. Requests carrying `Authorization` or `Cookie`, authenticated Starlette scopes, and FastAPI routes with dependencies, including ordinary custom-header authentication, bypass the unscoped default and always run the application, so route-level authentication cannot be skipped by a replay. Discovery covers FastAPI wrapped directly, behind route-transparent ASGI middleware, through a directly wrapped mount or router endpoint, or mounted under another ASGI application while keeping parent dependencies outside a mounted public app. Starlette authentication configured lazily on an app, on an individual route, inside an included router, or inside a mount is detected path by path, so a protected mount bypasses the default without disabling public siblings. FastAPI dependency overrides are followed through included and nested routers, including an override added, replaced, or removed after traffic starts; an outer include provider remains authoritative over conflicting child application providers, matching FastAPI's routing context. Response-cache declarations and explicit parent cache rules stop at middleware wrapped around or configured inside a mounted application, and at route-local middleware, so a parent hit cannot skip those boundaries. A required idempotency key is still enforced before that bypass. The built-in key includes canonical authority, scheme, and root path and uses structured field encoding, so host-routed tenants, mounts, and fields containing delimiters cannot cross-replay. The serialization is stored as a fixed 67-byte `v3:` SHA-256 key, so large incompressible queries remain safe for indexed PostgreSQL text columns. Its version is bumped so an upgraded or rolling-deployment process cannot read entries written by either vulnerable format; custom `key_maker` values remain unchanged.
* 🔒 Access logs retain query parameter names but mask every value. OAuth/OIDC tokens, authorization codes, presigned URL signatures, cloud credentials, and application-specific bearer capabilities no longer depend on an incomplete credential-name denylist to stay out of logs. URL and configuration redaction also recognizes qualified and camel-case password, key, token, credential, secret, and signature names, masks credential parameters anywhere in fragments, including path-prefixed assignments and nested redirects separated by raw or percent-encoded question marks, and fully masks ambiguous scheme-less or malformed-scheme credentials containing colons, commas, or extra `@` characters.
* 🔒 `SettingsValidationError` is raised even when the rejected value resists inspection. Deciding what to redact ran `isinstance` and rendered the value, both caller code, so a lazy proxy raising from `__class__` replaced the error with its own and skipped the redaction that keeps a credential out of the message. Every step of the scrub is total now. ([#782](https://github.com/grelinfo/grelmicro/pull/782))
...
* 🔒 Every error response carries `Cache-Control: no-store`, so a shared cache cannot serve one client's `429` to another, and `X-Content-Type-Options: nosniff`, because the body reflects the request path back in `instance`. ([#740](https://github.com/grelinfo/grelmicro/issues/740))
...
* 🐛 One bad key in a mounted ConfigMap stopped every other instance from reloading. `Match.exception` raised `TypeError`, which pydantic does not convert, so it escaped the reload loop and aborted the whole resync cycle. An empty `GREL_RETRY_{NAME}_WHEN` was enough to trigger it. The loop now skips any value a validator refuses, whatever it raises. ([#760](https://github.com/grelinfo/grelmicro/issues/760))
...
* 💥 The bare `GREL_{PATTERN}_` prefix is now the default for every instance of a pattern, not just the default instance. `GREL_LOCK_LEASE_DURATION=60` used to configure `Lock("default")` alone. It now sets the lease for every `Lock` that does not declare its own `GREL_LOCK_{NAME}_LEASE_DURATION`. A twelve-lock service tunes one variable instead of twelve. An app that relied on the bare variable reaching only the default instance must move that value to `GREL_LOCK_DEFAULT_...`, or accept the wider reach. ([#733](https://github.com/grelinfo/grelmicro/pull/733))

### Added

* ✨ `micro.describe()` returns a report of what the app is wired with: every component, its backend, the provider it borrows, and the checks that passed. Credential-like values are masked. `python -m grelmicro check app:micro` renders it and exits non-zero on a failing check, so CI can gate on the wiring. ([#733](https://github.com/grelinfo/grelmicro/pull/733))
* ✨ A provider report names the kinds it **declines**, not only the ones it serves. `uses=[redis]` leaving the outbox unwired was silently swallowed as a `NotImplementedError`, and is now the first thing the report shows. ([#733](https://github.com/grelinfo/grelmicro/pull/733))
* ✨ `micro.install(app)` resolves the framework through a new `grelmicro.integrations` entry-point group instead of sniffing attributes. A third-party package can now ship an integration, and a `FastAPI` subclass declared in your own package resolves correctly because the lookup walks the class's MRO. ([#733](https://github.com/grelinfo/grelmicro/pull/733))
* ✨ `micro.fake()` swaps every backed component onto an in-process store for a block, so a test runs the real code paths against real primitives with no Redis and no Postgres. A fixture can now open the app you actually ship and fake only its backends. ([#733](https://github.com/grelinfo/grelmicro/pull/733))
* ✨ `micro.get(Cache)` keeps the component type. Pass the class to resolve typed, the kind string to resolve a third-party component as `Any`. `micro.get("cache")` and `micro.<kind>` are unchanged. ([#733](https://github.com/grelinfo/grelmicro/pull/733))
* ✨ Every startup diagnostic carries a stable code, such as `backend-scope`. The code trails the message, travels as a `diagnostic` field on the log record, and links to a section in the new [Diagnostics](diagnostics.md) reference. Each warning also gets its own category (`BackendScopeWarning`, `EnvLoadOffWarning`, ...) under `GrelmicroConfigWarning`, so one diagnostic is silenced by category without matching message text and without silencing the rest. ([#733](https://github.com/grelinfo/grelmicro/pull/733))

### Fixed

* 🐛 A bare backend matching two protocols resolved by the order of two `if` statements. `runtime_checkable` compares member names only, so every `CircuitBreakerBackend` also matched `RateLimiterBackend`, and swapping the checks would have silently registered every breaker as a rate limiter. The most specific protocol now wins, and a backend matching two unrelated protocols raises `AmbiguousBackendError`. ([#733](https://github.com/grelinfo/grelmicro/pull/733))
* 🐛 A `TypeError` raised inside a zero-argument `__init__` was reported as "needs constructor arguments", suggesting a fix that could not work. The arity check now reads the signature before calling, so the real error propagates untouched. ([#733](https://github.com/grelinfo/grelmicro/pull/733))

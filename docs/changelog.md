...
### Added
* ✨ `JWTVerifier` verifies the bearer token a caller presents, against a fixed key set and claim policy. Signature, `exp`, `nbf`, `aud` and `iss` are checked in a compiled core, so a verification costs about 12 microseconds where a pure-Python library costs 46. Install it with `pip install "grelmicro[jwt]"`.

...
* 🐛 A `Bulkhead` opens its `uses=` scope once per app, not once per process. The scope was recorded on the bulkhead itself and never released, so a second `Grelmicro` app in the same process entered nothing at all, and an entry that had failed part way was worse: the next app resumed at the item that failed and skipped the ones before it. The app that opens the scope now holds the record of it and takes it away on shutdown, so the next app enters every item again from the start. Whether a re-entered provider comes back usable is up to that provider: `PostgresProvider` and `SqliteProvider` rebuild their pool, `RedisProvider` and `ValkeyProvider` do not, exactly as in `Grelmicro(uses=[...])`. Three consequences to know about: entering the bulkhead once its app has shut down raises `OutOfContextError` instead of silently using closed providers, sharing one `uses=` bulkhead between two apps that run at once (`allow_multiple=True`) raises instead of letting the first to exit close the items under the other, and a task whose context predates `async with micro:` raises `NoActiveAppError` every time rather than only when it enters first. ([#791](https://github.com/grelinfo/grelmicro/issues/791))

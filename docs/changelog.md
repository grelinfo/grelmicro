...
### Fixed
* 🐛 A record written through the standard library reaches the format the app configured on every backend. `loguru` and `structlog` installed no root handler, so grelmicro's own component records, and every dependency logging through `logging`, fell through to `logging.lastResort`: the bare message on stderr at warning and above, and nothing at all below it. ([#806](https://github.com/grelinfo/grelmicro/issues/806))

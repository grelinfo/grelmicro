...
* 🔒 An idempotency key can no longer replay a private response across callers when no identity-aware `key_maker` is configured. Requests carrying `Authorization` or `Cookie` bypass the unscoped default and always run the application, so route-level authentication cannot be skipped by a replay.
* 🔒 Access logs retain query parameter names but mask every value. OAuth/OIDC tokens, authorization codes, presigned URL signatures, cloud credentials, and application-specific bearer capabilities no longer depend on an incomplete credential-name denylist to stay out of logs. URL and configuration redaction also recognizes separator-delimited token, credential, secret, and signature names.

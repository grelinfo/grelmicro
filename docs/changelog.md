...
### Added
* ✨ Live reload tunes what a request costs, never what it is protected by. `CachedResponses`, `RateLimitedRequests` and `AccessLog` are fully live. `ConditionalRequests` and `IdempotentRequests` fix their reach at startup, because taking a path out of either one silently removes a guarantee: a retry that runs twice, or a write that erases an update nobody is told about. A key a file may not change is reported and the rest of the file still applies. ([#813](https://github.com/grelinfo/grelmicro/issues/813))

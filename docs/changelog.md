...
* 🔒 An idempotency key can no longer replay a private response across callers when no identity-aware `key_maker` is configured. Requests carrying `Authorization` or `Cookie` bypass the unscoped default and always run the application, so route-level authentication cannot be skipped by a replay. The built-in key format is versioned so an upgraded or rolling-deployment process cannot read unscoped entries written by the vulnerable format.

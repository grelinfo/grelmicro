...
* ✨ `grelmicro.http` ships `ProblemDetail` and `TMFError`, the Pydantic models for each body, `RenderedError` for what an integration writes, and `send_error(send, rendered)` to write one from a pure-ASGI middleware. ([#740](https://github.com/grelinfo/grelmicro/issues/740))

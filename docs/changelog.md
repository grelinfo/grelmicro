...
* 🐛 The OpenAPI annotation for idempotency follows `include=` and `exclude=`. An operation the middleware passes through was described with the idempotency header and the responses only a covered operation can get. A pattern written for a mounted app matches no declared route, and every operation of the covered methods is described rather than none. ([#688](https://github.com/grelinfo/grelmicro/issues/688))

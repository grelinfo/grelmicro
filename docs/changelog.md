...
### Added
* ✨ `TTLCache(name=...)` names a cache, and every cache metric carries it. `micro.cache.ttl()` inherits the component's name, so name each cache when an app builds more than one. It is a metric label rather than a configuration address: a cache still reads no environment variable. ([#691](https://github.com/grelinfo/grelmicro/issues/691))

...
* 💥 `Match.exception_message()` rejects a `contains=` that is not a string, and a `regex=` that is neither a string nor a compiled pattern, with `ValueError` at construction. An invalid regex is `ValueError` too: `re.error` is not one, so it escaped the validator conversion that only converts `ValueError`. Both used to build and then fail at match time. ([#782](https://github.com/grelinfo/grelmicro/pull/782))
...
* 🐛 Calling a matcher never raises. A predicate that raises, a class with a hostile `__instancecheck__`, an exception whose `__str__` or `__cause__` raises, and a return whose truth value cannot be read each read as no match and are reported once. `Retry` calls a matcher from an `except` block, so an error there replaced the very exception being handled. ([#782](https://github.com/grelinfo/grelmicro/pull/782))

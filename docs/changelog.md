...
* 🐛 A `Bulkhead` opens its `uses=` scope once per app, not once per process. The scope was recorded on the first app that entered it and never released, so a second `Grelmicro` app in the same process entered nothing and ran every call against providers closed with the first app. An entry that had failed part way was worse: the next app resumed at the item that failed and skipped the ones before it. The scope is now dropped as the app that opened it shuts down. ([#791](https://github.com/grelinfo/grelmicro/issues/791))

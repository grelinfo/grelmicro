...
* 🐛 A `Bulkhead` resolves `uses=` the way `Grelmicro(uses=[...])` does, so `Bulkhead("reports", uses=[redis])` scopes what redis serves instead of overriding nothing. A bare backend becomes its Component, a bare Provider fills a default Component per kind, two bare Providers raise `AmbiguousProviderError`, and a Provider or bare backend the app already opened is never opened again. ([#791](https://github.com/grelinfo/grelmicro/issues/791))

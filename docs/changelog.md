...
### Added
* ✨ `AccessLog(enduser=True)` writes who called as `enduser.id`, the subject of an authenticated caller, so one record says who asked for what and what they got. Off by default, because a subject can be personal data. (PR_LINK)

...
* 🐛 Uvicorn's handlers are moved off the log queue when it is taken out. `Log` stops the writer on exit and gave the root logger back, but uvicorn's handlers kept the stopped one and wrote the shutdown lines inline. ([#737](https://github.com/grelinfo/grelmicro/issues/737))

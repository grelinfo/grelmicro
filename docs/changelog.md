...
* 🐛 A `Bulkhead` opens its `uses=` scope once per app, not once per process. The scope was recorded on the bulkhead itself and never released, so a second `Grelmicro` app in the same process entered nothing and ran every call against providers closed with the first app. An entry that had failed part way was worse: the next app resumed at the item that failed and skipped the ones before it. The app that opens the scope now holds the record of it and takes it away on shutdown. Two consequences to know about: entering the bulkhead once its app has shut down raises `OutOfContextError` instead of silently using closed providers, and a task whose context predates `async with micro:` raises `NoActiveAppError` every time rather than only when it enters first. ([#791](https://github.com/grelinfo/grelmicro/issues/791))

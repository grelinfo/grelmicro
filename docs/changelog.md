...
* ✅ The public API snapshot records default values, so a changed default fails CI. It used to blank them, on the stated grounds that they vary across the supported Python and Pydantic versions. Measured instead of assumed: all 392 defaults on the public surface repr identically on 3.12, 3.13 and 3.14, so only a sentinel's memory address needed normalising. `Retry(attempts=3)` becoming `attempts=5` is a behavioural break for every caller who never passed the argument, and it was invisible here. ([#758](https://github.com/grelinfo/grelmicro/issues/758))

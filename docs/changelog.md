...
* ✨ The replay marker means a replay and nothing else. A handler that sets the replay header itself loses it, on a fresh response as well as a replayed one, and the loss is logged once, so a client reading the header's presence is never told a fresh response is a replay. ([#688](https://github.com/grelinfo/grelmicro/issues/688))

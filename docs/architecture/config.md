...
| A JWT verifier reads its own prefix only, and never its key source, algorithm or key material | Its settings decide whose tokens are trusted, so a kind-wide variable would move trust between verifiers, and a variable choosing how a token is checked is a credential decision | A verifier setting stops deciding trust |

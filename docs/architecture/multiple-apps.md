...
Most components keep their state on the instance: `Sync`, `Cache`, `RateLimiters`, `CircuitBreakers`, and `HealthChecks` all hold their own backend, so two overlapping apps never touch each other. They run concurrently with no special handling, the same way a web framework lets you instantiate two `App` objects.
...
So grelmicro blocks the narrow case that is actually unsafe: opening a second app that owns `Log` or `Trace` while another app that owns one is still active raises `MultipleActiveAppsError`. Apps without those components overlap freely.

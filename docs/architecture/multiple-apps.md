...
Most components keep their state on the instance: `Coordination`, `Cache`, `RateLimiterComponent`, `CircuitBreakerComponent`, and `HealthChecks` all hold their own backend, so two overlapping apps never touch each other. They run concurrently with no special handling, the same way a web framework lets you instantiate two `App` objects.
